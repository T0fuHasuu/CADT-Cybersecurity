...
final class SemaphoreImpl implements Semaphore {
    ...
    public final boolean addAcquireToQueue(CancellableContinuation<? super Unit> cancellableContinuation) {
        ...
        while (true) {
            ...
            while (true) {
                if (segment.getId() >= access$getSEGMENT_SIZE$p && !segment.getRemoved()) {
                    obj = SegmentOrClosed.m1710constructorimpl(segment);
                    break;
                }
                Object access$getNextOrClosed = segment.getNextOrClosed();
                if (access$getNextOrClosed == ConcurrentLinkedListKt.CLOSED) {
                    obj = SegmentOrClosed.m1710constructorimpl(ConcurrentLinkedListKt.CLOSED);
                    break;
                }
            ...
            if (SegmentOrClosed.m1715isClosedimpl(obj)) {
                break;
            }
            Segment r6 = SegmentOrClosed.m1713getSegmentimpl(obj);
            while (true) {
        ...
        SemaphoreSegment semaphoreSegment2 = (SemaphoreSegment) SegmentOrClosed.m1713getSegmentimpl(obj);
